...
class ViTEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.cls_token = nn.Parameter(torch.zeros(1, 6, config.hidden_size))
        ...
        self.position_embeddings = nn.Parameter(torch.zeros(1, num_patches + 6, config.hidden_size))
    ...
    def interpolate_pos_encoding(self, embeddings, height, width):
        ...
        npatch = embeddings.shape[1] - 6
        N = self.position_embeddings.shape[1] - 6
        if npatch == N and height == width:
            return self.position_embeddings
        class_pos_embed = self.position_embeddings[:, 0:5]
        patch_pos_embed = self.position_embeddings[:, 6:]
